                            ...
                            <tr>
                                ...
                                <td style="min-width: 160px;">
                                    @if ($adventure->category)
                                        <strong>{{ $adventure->category->name }}</strong>
                                        <div>
                                            <small class="text-muted">{{ $adventure->category->slug }}</small>
                                        </div>
                                    @else
                                        <span class="text-danger">Category missing</span>
                                    @endif
                                </td>
                                <td style="min-width: 150px;">
                                    <div class="d-flex gap-2">
                                        @if ($adventure->image_one)
                                            <img
                                                src="{{ asset('storage/' . $adventure->image_one) }}"
                                                alt="{{ $adventure->title }}"
                                                width="62"
                                                height="52"
                                        ...
                                        @if ($adventure->image_two)
                                            <img
                                                src="{{ asset('storage/' . $adventure->image_two) }}"
                                                alt="{{ $adventure->title }}"
                                                width="62"
                                                height="52"
                                                style="object-fit: cover; border-radius: 7px;"
                                    ...
                                    @if (!$adventure->image_one && !$adventure->image_two)
                                ...
                                <td style="min-width: 260px;">
                                    @if ($adventure->description)
                                        {{ \Illuminate\Support\Str::limit($adventure->description, 100) }}
                                    @else
                                ...
                                <td>
                                    @php
                                        $featureCount = count($adventure->features ?? []);
                                    @endphp
                                    @if ($featureCount > 0)
                                        <span class="status-badge status-active">
                                            {{ $featureCount }} {{ \Illuminate\Support\Str::plural('Feature', $featureCount) }}
                                        </span>
                                    @else
                                ...
                                <td>
                                    <span class="status-badge {{ $adventure->status === 'active' ? 'status-active' : 'status-inactive' }}">
                                        {{ ucfirst($adventure->status) }}
                                    </span>
                                </td>
                                <td>
                                    <div class="ts-actions">
                                        <a href="{{ route('admin.adventures.edit', $adventure) }}"
                                            class="ts-action-btn ts-edit-btn">
                                            Edit
                                        </a>
                                        <form action="{{ route('admin.adventures.destroy', $adventure) }}"
                                            method="POST"
